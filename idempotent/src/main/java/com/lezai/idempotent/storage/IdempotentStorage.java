 ...
 *   <li>所有实现类在遇到存储访问异常时必须上抛
 *       {@link IdempotentStorageException} 或其子类,由调用方决定 fail-fast
 *       兜底策略(映射为 HTTP 503 + Retry-After 等)。</li>
...
public interface IdempotentStorage {
     ...
     * @throws IdempotentStorageException 存储访问异常,调用方不得将此异常视为"无记录"
     */
     ...
     * @throws IdempotentStorageException 存储访问异常
     */
     ...
     * @throws IdempotentStorageException 存储访问异常,不得视为 {@code false}
     */
